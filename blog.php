    ...
    <main class="page_content" style="margin-top: 140px;">
      <section
        ...
        <div class="container">
          ...
          <h1 class="page_title wow" data-splitting>Blog</h1>
        </div>
      </section>
                  ...
                  <?php while ($blog = mysqli_fetch_assoc($result)): ?>
                    ...
                    <div class="col-md-6 col-sm-6">
                      <div class="blog_item mb-5">
                        <a class="item_image" href="blog_details.php?blog_id=<?= $blog['id'] ?>">
                          <img style="height: 270px;" src="<?= './admin_area/blog_images/' . $blog['image'] ?>" alt="<?= htmlspecialchars($blog['title']) ?>" />
                        </a>
                        <div class="item_content">
                          <ul class="item_type_list unordered_list">
                            <li><a href="blog_categorized?category=<?= urlencode($blog['category']) ?>"><?= htmlspecialchars($cat1['category']) ?></a></li>
                          </ul>
                          <h3 class="item_title">
                            <a href="blog_details.php?blog_id=<?= $blog['id'] ?>">
                              <?= htmlspecialchars($blog['title']) ?>
                            </a>
                          </h3>
                          <ul class="post_meta unordered_list">
                            <li><?= date('F j, Y', strtotime($blog['date_created'])) ?></li>
                            <li><a href="blog_details.php?blog_id=<?= $blog['id'] ?>"><?= $blog['comments_count'] ?> Comments</a></li>
                          </ul>
